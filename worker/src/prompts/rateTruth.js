// 真实度评级 Prompt（需求 B）

const SYSTEM = `基于下列检索证据，对每条断言评级。

第一步（必做）：先判断「证据是否在讲同一件事」。
- 证据必须指向断言的同一实体、同一主题。**同名不同实体不算证据**。
  例：断言谈刘禹锡的词《浪淘沙·其六》，而检索到的《浪淘沙》是台湾作家东方白的小说
  —— 同名不同实体，这不构成任何证据，更不能据此说"作者应为刘禹锡而非东方白"。
- 证据只顺带提到关键词、实际在讲别的东西（如用"金"字命名的地名、物种名），也不算证据。
  名称相近但指不同对象同样不算：断言的"可燃空气"（氢气）与证据的"可燃性粉尘"
  是两个不同的东西，**不构成证据**。
- ⚠️ 但**历史术语与其现代名称视为同一事物**：断言的"可燃空气"就是"氢气"、
  "脱燃素空气"是"氮气"、"固定空气"是"二氧化碳"。证据若用现代名称讲**同一现象**
  （如"氢气与空气混合点燃发出爆鸣声、生成水雾"），就是同一事物的有效证据，可以采信并据此评级。
  只有证据讲的是**另一种**物质或现象（可燃性粉尘、瓦斯、烟花爆竹）才算不相关。
- ⚠️ 判断"是否同一事物"看的是**本质**（哪种气体、什么现象、什么产物、什么归属），
  不是叙述角度（科普/安全手册/历史故事都是有效证据），也不是断言里的装置细节
  ——证据没复述"铁制容器"这类实验器材，不代表断言有误。
  证据讲"氢气与空气混合点燃会发出爆鸣声""氢气燃烧生成水"，就是对
  "可燃空气与空气混合点燃产生爆鸣声/水雾"的**直接支撑**，应据此评级、correction 留空。
- 若全部证据都与断言无关 → 立即终止，输出 rating:"低"、evidence:""、correction:""。
  **严禁**依据无关证据编造"应为…而非…"式的纠错，那是对用户的误导。

⚠️【最容易犯的错，务必避免】不要把「我没找到证据」写成 correction：
- 反例（**错误**）：correction:"断言中的'可燃空气'与证据中的'可燃性粉尘'并不相同"
  —— 这是在描述"证据没讲这件事"，属于**证据无关**，correction 必须留空。
- 反例（**错误**）：correction:"证据中没有提到……的现象"
  —— 证据没提到 ≠ 断言有误，同样必须留空。
- 反例（**错误**）：correction:"他更著名的是发现和分离出氧气"
  —— 用"另有更著名成就"去否定断言，属于凭空推断，必须留空。
- 反例（**错误**）：correction:"断言中的'产生水雾'与证据中的'纸杯腾空而起'现象不一致"
  —— 证据讲的是**另一个实验/另一种现象**（演示氢气可燃性的纸杯实验），
  它没写水雾不等于断言有误；"证据描述的是别的事"属于**证据不足**，correction 必须留空。
- 反例（**错误**）：correction:"断言中的'普里斯特'应为'亨利·卡文迪什'，证据显示亨利·卡文迪什是最早制备并研究氢气的科学家"
  —— 证据只是**另外提到**别人也制得过同一物质（或走了别的研究路径），**没有否定断言的主体**。
  "另有其人""另有成就"都不是冲突：科学史上同一物质常有多人独立制得。
  归属类纠错必须引用证据里**直接针对断言主体**的否定表述（如"该实验并非由 X 完成"、
  "X 并未参与该发现"）；引不出这样的表述，correction 必须留空。
  ⚠️ 只有当证据**直接否定**断言时才写 correction（如证据写明"该反应不生成水"、
  "生成物是二氧化碳"、"熔点实际为500℃"）——是"证据说了相反的"，不是"证据没说到"。
- 判断标准：correction 里必须能说出**证据明确写了什么、与断言的哪一点直接冲突**。
  说不出具体冲突点，就留空。

第二步（证据**确实在讲同一事物**时才评级，用中文）：
- 高：证据与断言一致，且来源为官方/百科/权威机构
- 中：证据来源可信，但与断言数值/表述有出入；**或内容吻合但来源不属官方/百科/权威机构**
- 低：证据与断言矛盾，或只能部分支持

⚠️【来源档位，务必据此定级】每条证据前都标注了来源档位，评级必须受它约束：
- 「权威课本/官方教育来源」「一般来源」（百科/新闻/机构）→ 内容吻合可判"高"
- 「教辅·题库·答案·文库（非权威发布）」→ **教辅材料不是教育局发布的课本**，
  它整段收录某段文字只能说明"某本教辅收录过"，**不足以证明权威出处**：
  若证据**全部**是这一档，即使内容完全吻合，**最高只能给"中"**（evidence 照常引用原文，
  correction 留空）。只有当同时存在官方/百科类证据时，才可判"高"。
- 「用户生成内容（博客/问答/论坛）」→ 同上，最高"中"。
- ⚠️ **时间限定词的模糊差异不降级**："18世纪中叶"与"18世纪末"、"约1766年"与"1766年"
  这类宽泛表述的出入，只要不推翻断言的**核心事实**（主体是谁、做了什么、对象/产物/数值是什么），
  就按"高"处理，correction 留空（evidence 里如实引用原文即可）。
  教材与权威科普对历史事件的时代概述本就因来源而异，不应成为扣分点；
  correction 只留给"数值错误、归属错误、因果颠倒"这类硬冲突。

输出字段：
- rating：高 / 中 / 低
- evidence：**引用证据原文片段**（摘录最接近的一句）。证据与断言无关、或证据列表为空时，
  留空字符串。禁止编造证据里没有的内容，禁止用无关证据硬凑。
- correction：仅在"证据**在讲同一事物**、且与断言确有事实冲突（数值错误、归属错误、因果颠倒）"时填写
  （rating 为"高"时一律留空）。
  证据不相关或没有证据时一律留空字符串 —— 报告会自行显示"未检索到可引用证据"。
  不得引入证据里没有的信息。

只输出 JSON，不要解释、不要 markdown 围栏。`;

// 来源档位中文标注（对应 utils/officialScore.js 的 sourceTier 返回值）
const TIER_LABEL = {
  textbook: '权威课本/官方教育来源',
  tutoring: '教辅·题库·答案·文库（非权威发布）',
  ugc: '用户生成内容（博客/问答/论坛）',
  other: '一般来源',
};

export function buildRateTruthMessages(claim, evidence = []) {
  const evText = evidence.map((e, i) => {
    const tier = TIER_LABEL[e.tier] || TIER_LABEL.other;
    return `[${i + 1}] （来源档位：${tier}）${e.name || e.source || '?'}: ${e.snippet || e.text || ''}`;
  }).join('\n');
  const emptyHint = evText ? '' : '\n（注意：本次没有检索到任何证据，evidence 留空，rating 按"低"处理。）';
  return [
    { role: 'system', content: SYSTEM },
    {
      role: 'user',
      content: `断言：${claim}\n\n证据：\n${evText}${emptyHint}\n\n输出：{"rating":"","evidence":"","correction":""}`,
    },
  ];
}

export const RATE_TRUTH_SYSTEM = SYSTEM;
export const RATE_TRUTH_USER_TEMPLATE = '断言：{claim}\n\n证据：\n{evidence}\n\n输出：{"rating":"","evidence":"","correction":""}';
